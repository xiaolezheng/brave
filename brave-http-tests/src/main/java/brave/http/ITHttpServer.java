...
public abstract class ITHttpServer {
  ...
  public void setup() throws Exception {
    init(tracing = tracingBuilder(Sampler.ALWAYS_SAMPLE).build());
  }

  void init(Tracing tracing) throws Exception {
    init(tracing, null);
  }

  /** recreate the server if needed */
  protected abstract void init(Tracing tracing, @Nullable Supplier<String> spanName)
      throws Exception;
  ...
  public void samplingDisabled() throws Exception {
    init(tracing = tracingBuilder(Sampler.NEVER_SAMPLE).build());
